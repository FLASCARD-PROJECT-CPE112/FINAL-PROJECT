...
typedef struct 
{
    ...
    char password[100]; //store password
}User;
...
{
    int ID; //flashcard id
    char question[Max]; //flashcard question
    char answer[Max]; //flashcard answer
    char category[100]; //main category name
    char subcategory[100]; //subcategory name
    int score; 
    struct card *next; //pointer to the next flashcard in linked list
};


struct HashTableEntry 
{
    int key; //key > use in hash table (flashcard ID)
    struct card *card; //pointer to flashcard in hash
};
...
{
    FILE *file = fopen(filename, "w"); //w = overwrite
    ...
    while (temp != NULL) //loop run thru flashcard
    {
        ...
        temp = temp->next; //temp point to next flashcard (move to next flashcard in the list)
    }
...
void addFlashcard() 
{
    ...
    while (1) //check user can press enter only 1-10
    {
        char input[100];
        printf("Enter score (1-10): "); //user input score
        fgets(input, sizeof(input), stdin);

        if (sscanf(input, "%d", &score) == 1 && score >= 1 && score <= 10)
        {
            break; //exit loop
        }
        else
    ...
    if (head == NULL) //add new flashcard to the end of linked list
    {
        head = newCard; // 1st flashcard
    }
    ...
    else  
    {
        ...
        temp->next = newCard; //add new flashcard at the end
    }

    int index = hashFunction(id); //store flashcard in hash table and use id to find index(flashcard)
    hashTable[index].key = id;
    hashTable[index].card = newCard;

    printf("Flashcard added successfully with ID %d.\n", id); 

    //save the update flashcard list for each user 
    char filename[200];
...
struct card *searchByID(int id) 
{
    ...
    if (hashTable[index].key == id) //key match the given ID
    {
...
void editFlashcard() 
{
    ...
    //show the current detail of the select flashcard
    printf("\nFlashcard found:\n");
    printf("ID: %d\nCategory: %s\nSubcategory: %s\nQuestion: %s\nAnswer: %s\nScore: %d\n\n", //show flsahcard detail
           cardToEdit->ID,
           cardToEdit->category,
    ...
    if (strlen(input) > 0) //check user input score correct
    {
        ...
        {
            cardToEdit->score = newScore; //vaild score > update
        }
...
void deleteFlashcard() 
{
    ...
    printf("\nEnter the ID of the flashcard to delete: "); //user input ID of flashcard to delete
    fgets(input, sizeof(input), stdin);
    input[strcspn(input, "\n")] = '\0'; 
    ...
    // show all detail of flashcards to delete
    printf("\nFlashcard found:\n");
    printf("ID: %d\nCategory: %s\nSubcategory: %s\nQuestion: %s\nAnswer: %s\nScore: %d\n",
    ...
    if (temp2 == NULL) //if not found list
    {
    ...
    {
        prev->next = temp2->next; //delete any other node
    }
    ...
    {
        hashTable[index].key = -1; //reset key
...
{
    int count = 0;
    struct card *temp = head; //count start from haed of list 
    while (temp) 
    {
        count++;
        temp = temp->next; //move to the next flashcard
    }
    return count;
}
...
int totalScore() //count flashcard score in linked list
{
    ...
    while (temp) 
    {
        sum += temp->score; //add score of current flashcard
        temp = temp->next;
    }
...
void listCategories() 
{
    ...
    while (temp != NULL) 
    {
        char combineCate[200];
        sprintf(combineCate, "%s/%s", temp->category, temp->subcategory); //combine cate/subcate into one string

        int duplicate = 0; 
        for (int i = 0; i < count; i++) 
        {
            if (strcmp(shown[i], combineCate) == 0)  //check combination has already been show
            {
                duplicate = 1;
                break;
            }
        ...
        if (!duplicate) 
        {
            strcpy(shown[count], combineCate);
            count++;
            printf("- %s / %s\n", temp->category, temp->subcategory);
        }
...
void practiceFlashcards() 
{
    ...
    int total = countFlashcards(); //check flashcard exist or not
    if (total == 0) 
    {
        printf("No flashcards available.\n");
        return;
...
void loadFlashcardsFromCSV(const char *filename) 
{
    ...
    while (fgets(line, sizeof(line), file)) 
    {
        ...
        else 
        {
            ...
            temp->next = newCard; //add new card at the end
        }
...
int checkUser(const char *username)
{
    ...
    while (fgets(line, sizeof(line), file)) //read each from file
    {
        sscanf(line, "%[^,],%[^\n]", fileuser, filepass);

        if (strcmp(fileuser, username) == 0) //compare username 
        {
            fclose(file);
            return 1;
        }
...
int loginUser()
{
    ...
    while (fgets(line, sizeof(line), file))
    {
        ...
        if(strcmp(username, fileuser) == 0) //username match in csv filename
        {
            found = 1;

            if (strcmp(password, filepass) == 0) //password also match
            {
